...
public class Main {
    ...
    private static void onOff(int[] switchArr, int[][] studentsArr) {
        for (int i = 0; i < studentsArr.length; i++) {
            if (studentsArr[i][0] == 1) {
                ...
                if (studentsArr[i][1] == 1) {
                    ...
                    if (studentsArr[i][1] > switchArr.length / 2) {
                        int idx = 0;
                        for (int j = 1; j <= switchArr.length - studentsArr[i][1]; j++) {
                            //좌우대칭 확인할 차례
                            //범위를 넘어서면 종료
                            if (switchArr[studentsArr[i][1] + j - 1] != switchArr[studentsArr[i][1] - j - 1]) break;
                            idx = j;
                        }
                        for (int j = studentsArr[i][1] - 1 - idx; j <= studentsArr[i][1] - 1 + idx; j++) {
                            switchArr[j] = switchArr[j] == 0 ? 1 : 0;
                        }
                    } else {
                        int idx = 0;
                        for (int j = 1; j < studentsArr[i][1]; j++) {
                            //좌우대칭 확인할 차례
                            //범위를 넘어서면 종료
                            if (switchArr[studentsArr[i][1] + j - 1] != switchArr[studentsArr[i][1] - j - 1]) break;
                            idx = j;
        ...
        for (int i = 0; i < switchArr.length; i++) {
            if (i == 0) System.out.print(switchArr[i] + " ");
            else if(i % 20 != 0) System.out.print(switchArr[i] + " ");
            else {
                System.out.println();
