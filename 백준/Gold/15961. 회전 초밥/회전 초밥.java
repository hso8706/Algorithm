...
//            int cntIdx = i-(k-1); // cnt 배열 인덱스, k-1부터 0이 되는 인덱스 => 같은 반복문에 넣었기 때문에 따로 해줘야함
            int leftIdx = i-k;
...
//                if(!temp.contains(temp.pollFirst())) cnt[cntIdx] = cnt[cntIdx-1]-1; // left 제거, 제거되는 값이 중복값이었는지 확인, 중복이 아니었다면 cnt 값 감소
//                else cnt[cntIdx] = cnt[cntIdx-1]; // 제거되는 값이 중복이었다면 cnt 값 유지
                // left 제거 과정
