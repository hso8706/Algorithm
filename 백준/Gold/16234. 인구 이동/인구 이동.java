...
//    static Map<Integer, Integer> sumMap = new HashMap<>(); // 연합의 인구수 총합을 각각 저장
...
//        Map<Integer, Integer> sumMap = new HashMap<>(); // 연합의 인구수 총합을 각각 저장 => static 으로 변경 및 clear() 사용
        int link = 1; // 연결된 나라를 파악하기 위함
        for (int x = 0; x < N; x++) {
            for (int y = 0; y < N; y++) {
                if (!visited[x][y]) {
                    makeUnion(new Pair(x, y), link); // 시간 : 5_000_000에서 완탐이 되어버리면 125억이됨.
//                    sumMap.put(link, aftMov);
                    link++;
                }
            }
        }
        if (link == (N * N) + 1) {
            flag = false; // link 변화가 없었으면 인구 이동이 없었음(false)
            return;
        }

//        for (int k = 1; k < link; k++) {
//            for (int i = 0; i < N; i++) {
//                for (int j = 0; j < N; j++) {
//                    if (union[i][j] == k) {
//                        map[i][j] = sumMap.get(k);
//                    }
//                }
//            }
//        }
//        sumMap.clear();
    ...
    private static void makeUnion(Pair start, int link) {
        // bfs 메소드
        // 4방 탐색으로 갭 차이 조건 확인
        int sum = 0;
        int cnt = 0;
